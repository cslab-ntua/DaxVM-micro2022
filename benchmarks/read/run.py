...
bins = [READ, MMAP, MMAP_POPULATE, MMAP_DAXVM]
iosize = [4096, 4096*4, 4096*16, 4096*64, 4096*256, 4096*512*2,
          4096*512*8, 4096*512*32, 4096*512*128, 4096*512*512, 4096*512*512*4]
iosize = [4096*512*512]
aging = ["aged"]
prezeroing = ["off"]
dirty = ["on"]
    ...
    for io in iosize:
        ...
        for pre in prezeroing:
            for _bin in bins:
                if _bin >= MMAP_DAXVM and pre == "off":
                    for _d in dirty:
                        out.write(("%3f," % (statistics.mean(results[_random][_a][r][io][_bin][1][pre][_d])/statistics.mean(
                            results[_random][_a][r][io][READ][1]["off"]))).rjust(20))
                else:
                    out.write(("%3f," % (statistics.mean(results[_random][_a][r][io][_bin][1][pre])/statistics.mean(
                        results[_random][_a][r][io][READ][1]["off"]))).rjust(20))
        out.write("\n")
    out.write("\n\n\n")
    out.close()
    ...
    for io in iosize:
        ...
        for pre in prezeroing:
            for _bin in bins:
                if _bin >= MMAP_DAXVM and pre == "off":
                    for _d in dirty:
                        out.write(("%3f," % (statistics.stdev(results[_random][_a][r][io][_bin][1][pre][_d]))).rjust(20))
                else:
                    out.write(("%3f," % (statistics.stdev(results[_random][_a][r][io][_bin][1][pre]))).rjust(20))
        out.write("\n")
    out.write("\n\n\n")
    out.close()
