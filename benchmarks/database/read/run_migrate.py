...
def call_normal(core, _bin, io, r, _random, pre, _a):
    ...
    for _run in range(0, runs):
        ...
        exe = "taskset -c 0-%d ./read %d %d %d %d %d > result.txt" % (core-1, core, _bin, io, _r, _random)
        print(exe)
        os.system(exe)
...
def call_daxvm(core, _bin, io, r, _random, pre, _a, pmem):
    ...
    for _d in types:
        ...
        for _run in range(0, runs):
            os.system("rm /mnt/nvmm1/micro/large_test.txt")
            os.system("../../../Linux/tools/daxvm/profiler-migrater/profile -u xalverti >/dev/null &")
            exe = "taskset -c 0-%d ./read %d %d %d %d %d > result.txt" % (core-1, core, _bin, io, _r, _random)
            print(exe)
            os.system(exe)
                ...
                if _bin >= MMAP_DAXVM and pre == "off":
                    for _d in dirty:
                        out.write(("%3f," % (statistics.mean(results[_random][_a][r][io][_bin][1][pre][_d])/statistics.mean(
                            results[_random][_a][r][io][READ][1]["on"]))).rjust(20))
                else:
...
os.system("echo 0 > /proc/fs/ext4/%s/daxvm_page_tables_on" % pmem)
for _random in random:
    for _a in aging:
        if "nohuge" == _a:
